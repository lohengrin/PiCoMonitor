#include "st7789Ex.hpp"

#include <cstdlib>
#include <math.h>

namespace PiCoMonitor {
  uint8_t madctl;
  ...
  void ST7789EX::common_init() {
    ...
    // Common init
    command((screen_type==PIMORONI)?reg::TEON:reg::TEOFF);  // enable frame sync signal if used
    ...
    command((screen_type==PIMORONI)?reg::INVON:reg::INVOFF);   // set inversion mode
  ...
  void ST7789EX::configure_display(pimoroni::Rotation rotate) {

    if(rotate == pimoroni::ROTATE_90 || rotate == pimoroni::ROTATE_270) {
    ...
    if(width == 240 && height == 135) {
      ...
      raset[0] = 52;   // 135 rows
      raset[1] = 52 + height - 1;
      if (rotate == pimoroni::ROTATE_0) {
        raset[0] += 1;
        raset[1] += 1;
      }
      madctl = rotate == pimoroni::ROTATE_180 ? MADCTL::ROW_ORDER : MADCTL::COL_ORDER;
    ...
    if(width == 135 && height == 240) {
      ...
      raset[1] = 40 + height - 1;
      madctl = 0;
      if (rotate == pimoroni::ROTATE_90) {
        caset[0] += 1;
        caset[1] += 1;
        madctl = MADCTL::COL_ORDER | MADCTL::ROW_ORDER;
      }
      madctl = rotate == pimoroni::ROTATE_90 ? (MADCTL::COL_ORDER | MADCTL::ROW_ORDER) : 0;
    }
    ...
    if(width == 320 && height == 240) {
      ...
      raset[0] = 0;
      raset[1] = 239;
      madctl = (rotate == pimoroni::ROTATE_180 || rotate == pimoroni::ROTATE_90) ? MADCTL::ROW_ORDER : MADCTL::COL_ORDER;
      madctl |= MADCTL::SWAP_XY | MADCTL::SCAN_ORDER;
    }
    ...
    if(width == 240 && height == 320) {
      ...
      raset[0] = 0;
      raset[1] = 319;
      madctl = (rotate == pimoroni::ROTATE_180 || rotate == pimoroni::ROTATE_90) ? (MADCTL::COL_ORDER | MADCTL::ROW_ORDER) : 0;
    }
  ...
  void ST7789EX::write_blocking_dma(const uint8_t *src, size_t len) {
  ...
  void ST7789EX::write_blocking_parallel(const uint8_t *src, size_t len) {
  ...
  void ST7789EX::command(uint8_t command, size_t len, const char *data) {
  ...
  void ST7789EX::update(pimoroni::PicoGraphics *graphics) {
    uint8_t cmd = reg::RAMWR;

    if(graphics->pen_type == pimoroni::PicoGraphics::PEN_RGB565) { // Display buffer is screen native
      ...
      graphics->frame_convert(pimoroni::PicoGraphics::PEN_RGB565, [this](void *data, size_t length) {
        if (length > 0) {
  ...
  void ST7789EX::set_backlight(uint8_t brightness) {
