...
#ifndef NO_QSTR
#include "st7789Ex_parallel.pio.h"
#endif
...
namespace PiCoMonitor {
  ...
  class ST7789EX : public pimoroni::DisplayDriver {
    spi_inst_t *spi = PIMORONI_SPI_DEFAULT_INSTANCE;

  public:

    enum Type {
      PIMORONI, // Original Pimoroni screen
      ELECROW   // ELECROW CrowPanel HMI 2.8 only
    };
  ...
  private:
    ...
    uint dc;
    uint wr_sck;
    uint rd_sck = pimoroni::PIN_UNUSED;
    uint d0;
    uint bl;
    uint vsync  = pimoroni::PIN_UNUSED; // only available on some products
    uint parallel_sm;
    PIO parallel_pio;
    ...
    // The ST7789EX requires 16 ns between SPI rising edges.
    // 16 ns = 62,500,000 Hz
  ...
  public:
    // Parallel init
    ST7789EX(uint16_t width, uint16_t height, pimoroni::Rotation rotation, pimoroni::ParallelPins pins) :
      DisplayDriver(width, height, rotation),
      spi(nullptr), round(false),
      cs(pins.cs), dc(pins.dc), wr_sck(pins.wr_sck), rd_sck(pins.rd_sck), d0(pins.d0), bl(pins.bl),
      screen_type(PIMORONI) {
    ...
    // Serial init
    ST7789EX(uint16_t width, uint16_t height, pimoroni::Rotation rotation, bool round, Type type, pimoroni::SPIPins pins) :
      DisplayDriver(width, height, rotation),
    ...
    void update(pimoroni::PicoGraphics *graphics) override;
    void set_backlight(uint8_t brightness) override;

  private:
    void common_init();
    void configure_display(pimoroni::Rotation rotate);
