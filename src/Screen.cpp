...
//----------------------------------------------------------------------
Screen::Screen(int width, int height, std::unique_ptr<PicoGraphics> g) :
    graphics(std::move(g)),
    myWidth(width),
    myHeight(height)
{
...
//----------------------------------------------------------------------
void Screen::set_target_backlight(uint8_t val)
{
    myTargetBacklight = val;
    set_backlight(val);
}
